...
const Edit = ({ history, hotels, match: { params: { _id } } }) => {
  ...
  async function update() {
    try {
      ...
      let found = await db.Residence.findOne({ roomNo: doc.roomNo, hotel: doc.hotel })
      ...
      <main>
        ...
        <div className='row'>
          ...
          <div className='row'>
            ...
            <div className='col-4'>
              <Input style={{ textAlign: 'center' }} type='text' caption='لايمكن تكرار رقم الغرفة!' label='رقم الجناح / الغرفة / الحظيرة' id='roomNo' placeholder='F01, G01, ...' defaultValue={state.roomNo} onChange={onChange} disabled />
            </div>
            <div className='col-4'>
              <Input min={0} lang="en-150" type='number' label='عدد الأسِرَّة' id='numberOfBeds' placeholder='مثلاً ... 1,2,4' defaultValue={state.numberOfBeds} onChange={onChange} />
            </div>
