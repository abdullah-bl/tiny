...
export const CommandSchema = Joi.object({
	...
	type: Joi.valid('صيانة', 'عزل', 'نظافة').error(
		Errors('يجب ان تحدد نوع الطلب !')
	),
...
export const ReservationSchema = Joi.object({
	id: Joi.number()
		.required()
		.error(Errors('الرقم العسكري او رقم الهوية مطلوب!')),
	name: Joi.string().required().error(Errors('يجب كتابة الاسم!')),
	rank: Joi.string().required().error(Errors(' يجب اختيار الرتبة')),
...
export const RoomSchema = Joi.object({
	...
	type: Joi.valid('غرفة', 'جناح', 'حظيرة')
		.default('غرفة')
		.error(Errors('يجب اختيار النوع !')),
	status: Joi.valid('غير شاغرة', 'شاغرة', 'صيانة', 'عزل', 'نظافة').default(
		'شاغرة'
	),
	note: Joi.string().allow(''),
})
